...
CustomPlotBar::CustomPlotBar(QWidget * parent)
    : QCustomPlot(parent)
    , mZoomMode(true)
    , mRubberBand(new QRubberBand(QRubberBand::Rectangle, this))
{}

CustomPlotBar::~CustomPlotBar()
{
    delete mRubberBand;
}

void CustomPlotBar::setZoomMode(bool mode)
{
    mZoomMode = mode;
}

void CustomPlotBar::mousePressEvent(QMouseEvent * event)
{
    if (mZoomMode)
    {
...
void CustomPlotBar::mouseMoveEvent(QMouseEvent * event)
{
...
void CustomPlotBar::mouseReleaseEvent(QMouseEvent * event)
{
    if (mRubberBand->isVisible())
    {
...
void CustomPlotBar::axisDoubleClick(QCPAxis * axis, QCPAxis::SelectablePart part, QMouseEvent * event)
{
    rescaleAxes(true);
    /*
    if (!graphCount())
